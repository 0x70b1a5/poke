use crate::kinode::process::poke::{ChatMessage, Request as ChatRequest, Response as ChatResponse, SendRequest};
...
fn handle_message (our: &Address) -> anyhow::Result<()> {
    ...
    print_to_terminal(0, "poke_test: a");
    ...
    // Send
    print_to_terminal(0, "poke_test: b");
...
fn init(our: Address) {
    ...
    loop {
        match handle_message(&our) {
            Ok(()) => {},
            Err(e) => {
                print_to_terminal(0, format!("poke_test: error: {e:?}").as_str());
