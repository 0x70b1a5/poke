...
fn init(our: Address) {
    ...
    let Ok(target) = Address::from_str(&args) else {
        println!("usage:\nsend:poke:template.os target");
        return;
    };

    let Ok(Ok(Message::Response { body, .. })) =
        Request::to((our.node(), ("poke", "poke", "template.os")))
            .body(
        ...
        println!("did not receive expected Response from poke:poke:template.os");
        return;
    };

    let Ok(ChatResponse::Send) = serde_json::from_slice(&body) else {
        println!("did not receive expected Ack from poke:poke:template.os");
        return;
    };
}
